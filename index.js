(function () {
	...
	var COLOR_ELEMENT_SELECTOR = 'span[data-type="color"]';
	function isColorElement(element) {
	  return element instanceof HTMLSpanElement && element.getAttribute('data-type') === 'color';
	}
	function getClosestParentColorElement(colorElement) {
	  if (isColorElement(colorElement)) {
	    var PARENT = $(colorElement).parents(COLOR_ELEMENT_SELECTOR).first()[0];
	    return PARENT;
	  }
	  throw TypeError("Child element must be a color element");
	}
	function getFurthestUnderlineOrStrikethroughAncestorElement(colorElement) {
	  if (isColorElement(colorElement) === false) {
	    throw TypeError("Must be a color element");
	  }
	  return $(colorElement).parents('u, s').last()[0];
	}
	function getInnerColorElements$1(parent) {
	...
	function separateColorElementFromParentColorElement(colorElement, parent) {
	  if (isColorElement(colorElement) === false) {
	    throw TypeError("Must be a color element");
	  }
	  if (isColorElement(parent) === false) {
	  ...
	  var NON_COLOR_ELEMENT_PARENTS = [];
	  $(colorElement).parents().each(function (_, p) {
	    if (p === parent) {
	  ...
	  // extract the slices
	  var LEFT_SLICE_FRAGMENT = LEFT_SLICE_RANGE.extractContents();
	  var MIDDLE_SLICE_FRAGMENT = MIDDLE_SLICE_RANGE.extractContents();
	  var RIGHT_SLICE_FRAGMENT = RIGHT_SLICE_RANGE.extractContents();
	  // wrap the contents of the child color element with the non-color-element parents
	  $(NON_COLOR_ELEMENT_PARENTS).each(function (_, element) {
	  ...
	  var NEW_PARENT_FRAGMENT = document.createDocumentFragment();
	  ...
	  $(NEW_PARENT_FRAGMENT).find('*').each(function (_, element) {
	...
	function separateColorElementFromUnderlineOrStrikethroughAncestorElement(colorElement, ancestor) {
	  if (isColorElement(colorElement) === false) {
	    throw TypeError("Must be a color element");
	  }
	  ...
	  var ANCESTOR_RANGE = document.createRange();
	  ANCESTOR_RANGE.selectNode(ancestor);
	  var ANCESTOR_FRAGMENT = ANCESTOR_RANGE.extractContents();
	  // get all the underline/strikethrough ancestors of the color element
	  var ANCESTORS = $(colorElement).parents('u, s');
	  ...
	  var LEFT_SLICE = LEFT_SLICE_RANGE.extractContents();
	  var MIDDLE_SLICE = MIDDLE_SLICE_RANGE.extractContents();
	  var RIGHT_SLICE = RIGHT_SLICE_RANGE.extractContents();
	  // wrap the contents of the color element with all the underline/strikethrough ancestors
	  var COLOR_ELEMENT_CONTENTS_RANGE = document.createRange();
	  COLOR_ELEMENT_CONTENTS_RANGE.selectNodeContents(colorElement);
	  ANCESTORS.each(function (_, ancestor) {
	...
	function removeInnerColorElements(parent) {
	  if (isColorElement(parent) === false) {
	    throw TypeError("Parent must be a color element");
	  }
	  var RANGE = document.createRange();
	  ...
	  $(FRAGMENT.children[0]).find(COLOR_ELEMENT_SELECTOR).each(function (_, innerColorElement) {
	    var INNER_COLOR_ELEMENT = $(innerColorElement);
	    INNER_COLOR_ELEMENT.replaceWith(INNER_COLOR_ELEMENT.contents());
	  });
	  RANGE.insertNode(FRAGMENT);
	}

	var FORMAT_ELEMENT_SELECTOR = 'b, i, u, s';
	function isFormatElement(element) {
	  return $.inArray(element.tagName.toLowerCase(), ['b', 'i', 'u', 's']) !== -1;
	}
	function getDuplicateDescendants(formatElement) {
	  if (isFormatElement(formatElement) === false) {
	    throw TypeError("Must be a format element");
	  }
	  var ELEMENT_TAG = formatElement.tagName.toLowerCase();
	  return $(formatElement).find(ELEMENT_TAG).toArray();
	}
	function getClosestDuplicateAncestor(formatElement) {
	  if (isFormatElement(formatElement) === false) {
	    throw TypeError("Must be a format element");
	  }
	  var ELEMENT_TAG = formatElement.tagName.toLowerCase();
	...
	function getInnerColorElements(formatElement) {
	  if (isFormatElement(formatElement) === false) {
	    throw TypeError("Must be a format element");
	  }
	  return $(formatElement).find(COLOR_ELEMENT_SELECTOR).toArray();
	}
	function separateInnerColorElementsFromParentFormatElement(formatElement) {
	  if (isFormatElement(formatElement) === false) {
	    throw TypeError("Must be a format element");
	  }
	  var FORMAT_ELEMENT_TAG = formatElement.tagName.toLowerCase();
	  var PARENT_RANGE = document.createRange();
	  PARENT_RANGE.selectNode(formatElement);
	  var PARENT_FRAGMENT = PARENT_RANGE.extractContents();
	  var INNER_COLOR_ELEMENTS = $(PARENT_FRAGMENT).find(COLOR_ELEMENT_SELECTOR).toArray();
	  var LAST_COLOR_ELEMENT_INDEX = INNER_COLOR_ELEMENTS.length - 1;
	  // divide the parent format element into color element & non-color-element slices
	  var FIRST_SLICE_RANGE = document.createRange();
	  FIRST_SLICE_RANGE.setStartBefore(formatElement);
	  FIRST_SLICE_RANGE.setEndBefore(INNER_COLOR_ELEMENTS[0]);
	  var LAST_SLICE_RANGE = document.createRange();
	  LAST_SLICE_RANGE.setStartAfter(INNER_COLOR_ELEMENTS[LAST_COLOR_ELEMENT_INDEX]);
	  LAST_SLICE_RANGE.setEndAfter(formatElement);
	  var RANGES_OF_MIDDLE_SLICES = [];
	  $(INNER_COLOR_ELEMENTS).each(function (index, colorElement) {
	    var COLOR_ELEMENT_RANGE = document.createRange();
	    ...
	    var CURRENT_COLOR_ELEMENT = $(colorElement);
	    var FORMAT_ELEMENT = $(document.createElement(FORMAT_ELEMENT_TAG));
	    ...
	    if (index < LAST_COLOR_ELEMENT_INDEX) {
	      var IN_BETWEEN_CONTENT_RANGE = document.createRange();
	  ...
	  $(RANGES_OF_MIDDLE_SLICES).each(function (_, range) {
	...
	function mergeSimilarAdjacentChildNodes(parent) {
	  var RANGE = document.createRange();
	  ...
	  var NODES_TO_DELETE = [];
	  var previous_node;
	  $(FRAGMENT).contents().each(function (_, node) {
	    if (previous_node === undefined) {
	      previous_node = node;
	      return; // skip first node
	    }
	    var current_node_marked_for_deletion = false;
	    // merge similar adjacent nodes
	    if (node instanceof Node) {
	      if (node instanceof HTMLElement && previous_node instanceof HTMLElement) {
	        if (isColorElement(node) && isColorElement(previous_node) && node.getAttribute('style') === previous_node.getAttribute('style')) {
	        ...
	        } else if (isFormatElement(node) && isFormatElement(previous_node) && node.tagName === previous_node.tagName) {
	  ...
	  if (NODES_TO_DELETE.length > 0) {
	    $(NODES_TO_DELETE).each(function (_, node) {
	      if (node instanceof Node) {
	        $(node).remove();
	      }
	    });
	...
	function deleteAllEmptyDescendants(parent) {
	  var RANGE = document.createRange();
	  RANGE.selectNodeContents(parent);
	  var FRAGMENT = RANGE.extractContents();
	  $(FRAGMENT).find('*').each(function (_, element) {
	    if (element.innerHTML.length === 0) {
	      $(element).remove();
	    }
	  });
	...
	function separateElementFromSpecificAncestor(element, ancestor) {
	...
	var RichTextEditor = /** @class */function () {
	  function RichTextEditor(textBoxId, emptyTextBox) {
	    if (emptyTextBox === void 0) {
	      emptyTextBox = true;
	    }
	    var _this = this;
	    this.TEXT_BOX_SELECTION_DATA = {
	      selection: null,
	      range: null
	    };
	    this.TEXT_BOX_LAST_SELECTION_DATA = {
	      lastSelection: null,
	      // HTML of selection after it has been styled
	      lastSelectionType: null // 'Caret' or 'Range'
	    };
	    this.TEXT_BOX_SELECTION_STYLES = {
	      formats: [],
	      textColor: undefined
	    };
	    ...
	    this.TEMPORARY_CONTAINER_CLASS = 'temporary-rte-container';
	    // PUBLIC
	    this.addEventListener = function (type, listener) {
	      if (_this.EVENT_LISTENERS[type] === undefined) {
	        throw RangeError("Invalid event type");
	      }
	      _this.EVENT_LISTENERS[type].push(listener);
	    };
	    ...
	    this.TEXT_BOX.on('mouseup', function (event) {
	      if (event.button === 0) {
	        // check if a caret selection was made and delete the caret selection element if it's empty
	        if (_this.__emptyCaretSelectionElementExists__()) {
	          $(_this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection).remove();
	        }
	        _this.__updateTextBoxSelectionData__();
	        _this.clearTextBoxLastSelectionData();
	        _this.__triggerEventListeners__('mouseup', {
	          metaData: event,
	          styles: _this.__getStylesInSelection__()
	        });
	      }
	    });
	    this.TEXT_BOX.on('mousedown', function (_) {
	      var _a;
	    ...
	    this.TEXT_BOX.on('keyup', function (event) {
	      /*
	      OBJECTIVES:
	      ...
	      */
	      ...
	      _this.__triggerEventListeners__('keyup', {
	        metaData: event,
	        styles: _this.__getStylesInSelection__()
	      });
	      ...
	      if (event.key.indexOf('Arrow') === -1) {
	        if (_this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection !== null && _this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType !== null) {
	          // check if a caret selection was made
	          if (_this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType === 'Caret') {
	            var element = _this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection;
	            if (isColorElement(element)) {
	              // check if the caret selection color element is inside of an existing color element and if so take it out
	              var PARENT_COLOR_ELEMENT = getClosestParentColorElement(element);
	              if (PARENT_COLOR_ELEMENT !== undefined) {
	                separateColorElementFromParentColorElement(element, PARENT_COLOR_ELEMENT);
	                _this.__selectAndPlaceCaretInsideElement__(element);
	              }
	              // check if the caret selection color element is inside of <u> or <s> elements and if so take it out of the furthest ancestor
	              var FURTHEST_UNDERLINE_OR_STRIKETHROUGH_ELEMENT = getFurthestUnderlineOrStrikethroughAncestorElement(element);
	              if (FURTHEST_UNDERLINE_OR_STRIKETHROUGH_ELEMENT !== undefined) {
	                separateColorElementFromUnderlineOrStrikethroughAncestorElement(element, FURTHEST_UNDERLINE_OR_STRIKETHROUGH_ELEMENT);
	                _this.__selectAndPlaceCaretInsideElement__(element);
	              }
	              ...
	              if (DESCENDANTS.length > 0) {
	                var INNERMOST_DESCENDANT = DESCENDANTS.get(-1);
	                element = INNERMOST_DESCENDANT;
	              }
	            } else if (isFormatElement(element)) {
	              ...
	              var CLOSEST_DUPLICATE_ANCESTOR = getClosestDuplicateAncestor(element);
	              if (CLOSEST_DUPLICATE_ANCESTOR !== undefined) {
	                var PARENT = element.parentElement;
	                FORMAT_ELEMENT.replaceWith(FORMAT_ELEMENT.contents());
	                mergeSimilarAdjacentChildNodes(PARENT);
	              }
	            }
	        ...
	        _this.TEXT_BOX.find(".".concat(_this.TEMPORARY_CONTAINER_CLASS)).each(function (_, container) {
	          ...
	          if (container.innerHTML.length > 0) {
	            ...
	            var LAST_NODE = CONTENTS.last();
	            CONTENTS.insertBefore(container);
	            // place the caret at the end of the last node
	            var NEW_SELECTION_RANGE = document.createRange();
	            var NODE_TYPE = LAST_NODE[0].nodeType;
	  ...
	  // PRIVATE
	  RichTextEditor.prototype.__updateTextBoxSelectionData__ = function () {
	    ...
	    if (WINDOW_SELECTION !== null && WINDOW_SELECTION.anchorNode !== null && WINDOW_SELECTION.focusNode !== null) {
	      var TEXT_BOX_ELEMENT = this.TEXT_BOX[0];
	      var SELECTION_INSIDE_TEXT_BOX = TEXT_BOX_ELEMENT.compareDocumentPosition(WINDOW_SELECTION.anchorNode) & Node.DOCUMENT_POSITION_CONTAINED_BY && TEXT_BOX_ELEMENT.compareDocumentPosition(WINDOW_SELECTION.focusNode) & Node.DOCUMENT_POSITION_CONTAINED_BY;
	      var SELECTION_IS_TEXT_BOX = WINDOW_SELECTION.anchorNode === TEXT_BOX_ELEMENT && WINDOW_SELECTION.focusNode === TEXT_BOX_ELEMENT;
	  ...
	  RichTextEditor.prototype.__getStylesInSelection__ = function () {
	    var STYLES = {
	      formats: [],
	      textColor: undefined
	    };
	    if (this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection !== null && this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType !== null && this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType === 'Range') {
	      var SELECTED_ELEMENT = $(this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection);
	      var TAG = SELECTED_ELEMENT[0].tagName.toLowerCase();
	      // get formats
	      SELECTED_ELEMENT.parents(FORMAT_ELEMENT_SELECTOR).each(function (_, element) {
	        STYLES.formats.push(element.tagName.toLowerCase());
	      });
	      if (isFormatElement(SELECTED_ELEMENT[0]) && $.inArray(TAG, STYLES.formats) === -1) {
	        STYLES.formats.push(TAG);
	      }
	      // get color
	      var color_string = undefined;
	      if (isColorElement(SELECTED_ELEMENT[0])) {
	        color_string = SELECTED_ELEMENT.css('color');
	      } else {
	        color_string = SELECTED_ELEMENT.parents(COLOR_ELEMENT_SELECTOR).first().css('color');
	      }
	      if (color_string !== undefined && color_string.length > 0) {
	        var RGB_STRINGS = color_string.replace(/[a-zA-Z)(]/g, '').split(/, ?/);
	      ...
	      var SELECTION = this.TEXT_BOX_SELECTION_DATA.selection;
	      if (SELECTION.anchorNode !== null && SELECTION.focusNode !== null && SELECTION.anchorNode === SELECTION.focusNode) {
	        // get formats
	        $(SELECTION.anchorNode).parents(FORMAT_ELEMENT_SELECTOR).each(function (_, element) {
	          STYLES.formats.push(element.tagName.toLowerCase());
	        });
	        if (SELECTION.anchorNode instanceof HTMLElement && isFormatElement(SELECTION.anchorNode)) {
	          var TAG = SELECTION.anchorNode.tagName.toLowerCase();
	        ...
	        // get color
	        var color_string = undefined;
	        if (SELECTION.anchorNode instanceof HTMLElement && isColorElement(SELECTION.anchorNode)) {
	          color_string = $(SELECTION.anchorNode).css('color');
	        } else {
	          color_string = $(SELECTION.anchorNode).parents(COLOR_ELEMENT_SELECTOR).first().css('color');
	        }
	  ...
	  RichTextEditor.prototype.__selectionInTextBoxExists__ = function () {
	    return this.TEXT_BOX_SELECTION_DATA.selection !== null && this.TEXT_BOX_SELECTION_DATA.range !== null;
	  };
	  RichTextEditor.prototype.__selectAndHighlightElement__ = function (element) {
	    var HIGHLIGHT_RANGE = document.createRange();
	  ...
	  RichTextEditor.prototype.__selectAndPlaceCaretInsideElement__ = function (element) {
	  ...
	  RichTextEditor.prototype.__emptyCaretSelectionElementExists__ = function () {
	    /*
	    NOTE:
	  ...
	  RichTextEditor.prototype.__applyFormat__ = function (formatElement) {
	    var _a, _b;
	    if (isFormatElement(formatElement) === false) {
	      throw TypeError("Format element must be one of the following: <b>, <i>, <u>, <s>");
	    }
	    var FORMAT_EVENT_DATA = {
	    ...
	    if (this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection !== null && this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType !== null) {
	      var SELECTED_ELEMENT = this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection;
	      var SELECTION_TYPE = this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType;
	      if (SELECTION_TYPE === 'Caret') {
	        ...
	        this.__triggerEventListeners__('format', FORMAT_EVENT_DATA);
	      } else if (SELECTION_TYPE === 'Range') {
	        var RANGE = document.createRange();
	        if (isColorElement(SELECTED_ELEMENT)) {
	          RANGE.selectNodeContents(SELECTED_ELEMENT);
	          RANGE.surroundContents(formatElement);
	          // save reference of formatted selection (in case user wants to make modifications to it before deselecting it)
	          this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection = formatElement;
	          ...
	          var DUPLICATE_DESCENDANTS = getDuplicateDescendants(formatElement);
	          if (DUPLICATE_DESCENDANTS.length > 0) {
	            $(DUPLICATE_DESCENDANTS).each(function (_, element) {
	              var DESCENDANT = $(element);
	              DESCENDANT.replaceWith(DESCENDANT.contents());
	            });
	            mergeSimilarAdjacentChildNodes(formatElement);
	          }
	          ...
	          var ELEMENT_TAG = formatElement.tagName.toLowerCase();
	          var INNER_COLOR_ELEMENTS = getInnerColorElements(formatElement);
	          if ((ELEMENT_TAG === 'u' || ELEMENT_TAG === 's') && INNER_COLOR_ELEMENTS.length > 0) {
	            separateInnerColorElementsFromParentFormatElement(formatElement);
	            mergeSimilarAdjacentChildNodes(formatElement);
	          }
	          // get rid of any empty elements left
	          deleteAllEmptyDescendants(this.TEXT_BOX[0]);
	      ...
	      var SELECTION_TYPE = (_a = this.TEXT_BOX_SELECTION_DATA.selection) === null || _a === void 0 ? void 0 : _a.type;
	      if (SELECTION_TYPE === 'Caret') {
	        ...
	        var ANCESTOR_FORMAT_ELEMENT_WITH_SAME_TAG = $((_b = this.TEXT_BOX_SELECTION_DATA.selection) === null || _b === void 0 ? void 0 : _b.anchorNode).parents(formatElement.tagName).first()[0];
	        if (ANCESTOR_FORMAT_ELEMENT_WITH_SAME_TAG === undefined) {
	          var SELECTION_RANGE = this.TEXT_BOX_SELECTION_DATA.range;
	          ...
	          var WINDOW_SELECTION = window.getSelection();
	          WINDOW_SELECTION.removeAllRanges();
	          WINDOW_SELECTION.addRange(this.TEXT_BOX_SELECTION_DATA.range);
	        }
	        this.__triggerEventListeners__('format', FORMAT_EVENT_DATA);
	      } else if (SELECTION_TYPE === 'Range') {
	        var SELECTION_RANGE = this.TEXT_BOX_SELECTION_DATA.range;
	        // convert selection to a fragment
	        var SELECTED_CONTENTS = SELECTION_RANGE.extractContents();
	        // put fragment inside of format element
	        FORMAT_ELEMENT.append(SELECTED_CONTENTS);
	        // replace selection with formatted contents
	        SELECTION_RANGE.insertNode(FORMAT_ELEMENT[0]);
	        // check if the new format element is inside of an ancestor format element with the same tag and if so undo the formatting
	        var CLOSEST_DUPLICATE_ANCESTOR = getClosestDuplicateAncestor(FORMAT_ELEMENT[0]);
	        if (CLOSEST_DUPLICATE_ANCESTOR === undefined) {
	          // check if the new format element has descendants with the same tag and if so remove them but keep their contents
	          var DUPLICATE_DESCENDANTS = getDuplicateDescendants(FORMAT_ELEMENT[0]);
	          if (DUPLICATE_DESCENDANTS.length > 0) {
	            $(DUPLICATE_DESCENDANTS).each(function (_, element) {
	              var DESCENDANT = $(element);
	              DESCENDANT.replaceWith(DESCENDANT.contents());
	            });
	            mergeSimilarAdjacentChildNodes(FORMAT_ELEMENT[0]);
	          }
	          // check if the new format element is an underline/strikethrough and if it has color element descendants, if so, separate them from the new format element
	          var ELEMENT_TAG = formatElement.tagName.toLowerCase();
	          var INNER_COLOR_ELEMENTS = getInnerColorElements(FORMAT_ELEMENT[0]);
	          if ((ELEMENT_TAG === 'u' || ELEMENT_TAG === 's') && INNER_COLOR_ELEMENTS.length > 0) {
	            separateInnerColorElementsFromParentFormatElement(FORMAT_ELEMENT[0]);
	            mergeSimilarAdjacentChildNodes(FORMAT_ELEMENT[0]);
	          }
	          ...
	          // undo formatting
	          var PARENT = FORMAT_ELEMENT.parent()[0];
	          FORMAT_ELEMENT.replaceWith(FORMAT_ELEMENT.contents());
	          mergeSimilarAdjacentChildNodes(PARENT);
	          this.__triggerEventListeners__('format', FORMAT_EVENT_DATA);
	        }
	  ...
	  RichTextEditor.prototype.__removeFormat__ = function (format) {
	    var _this = this;
	    var _a;
	    if ($.inArray(format, ['b', 'i', 'u', 's']) === -1) {
	      throw RangeError("Format must be one of the following: ".concat(FORMAT_ELEMENT_SELECTOR));
	    }
	    var FORMAT_EVENT_DATA = {
	      format: format,
	      action: 'remove'
	    ...
	    if (this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType === 'Range' && this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection !== null) {
	      ...
	      var HIGHLIGHTED_ELEMENT = $(this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection);
	      var TARGET = HIGHLIGHTED_ELEMENT.parents(format).first();
	      if (TARGET[0] === undefined) {
	        // target is the highlighted element
	        var CONTENTS = HIGHLIGHTED_ELEMENT.contents();
	        var PARENT = HIGHLIGHTED_ELEMENT.parent()[0];
	      ...
	      if (SELECTION_TYPE === 'Caret' && SELECTION_RANGE.startContainer instanceof HTMLElement && SELECTION_RANGE.startContainer.innerHTML === "\u200B") {
	            ...
	            $(PARENT.childNodes).each(function (_, node) {
	              if (node instanceof Node && node.nodeType === Node.TEXT_NODE && node.nodeValue !== null) {
	                ...
	                if (ZWSC_POSITION !== -1) {
	                  ...
	                  var NEW_CARET_SELECTION_RANGE = document.createRange();
	        ...
	        var TEMPORARY_CONTAINER = $(document.createElement('span'));
	        TEMPORARY_CONTAINER.addClass(this.TEMPORARY_CONTAINER_CLASS);
	        SELECTION_RANGE.surroundContents(TEMPORARY_CONTAINER[0]);
	        var styled_container_1 = undefined;
	        var target_format_element_1 = undefined;
	        TEMPORARY_CONTAINER.parents().each(function (_, parent) {
	          if (parent.tagName.toLowerCase() === format) {
	            // stop when the target format has been reached
	            target_format_element_1 = parent;
	            return false;
	          }
	          // recreate the selection's styles but exclude the target format
	          var STYLE_ELEMENT = parent.cloneNode();
	          if (isFormatElement(STYLE_ELEMENT) || isColorElement(STYLE_ELEMENT)) {
	            if (styled_container_1 === undefined) {
	              styled_container_1 = STYLE_ELEMENT;
	            } else {
	              STYLE_ELEMENT.appendChild(styled_container_1);
	              styled_container_1 = STYLE_ELEMENT;
	            }
	          }
	        });
	        ...
	        if (styled_container_1 !== undefined) {
	          TEMPORARY_CONTAINER.append(styled_container_1);
	        }
	        if (target_format_element_1 !== undefined) {
	          // separate temporary container from target format element
	          separateElementFromSpecificAncestor(TEMPORARY_CONTAINER[0], target_format_element_1);
	          if (SELECTION_TYPE === 'Caret') {
	            var INNERMOST_ELEMENT = TEMPORARY_CONTAINER.find('*').last();
	            ...
	            this.TEXT_BOX.find(format).each(function (_, element) {
	              if (element.innerHTML.length === 0) {
	                $(element).remove();
	              }
	            });
	            ...
	            var CONTENTS = TEMPORARY_CONTAINER.contents();
	            var FIRST_NODE = CONTENTS.first();
	            var LAST_NODE = CONTENTS.last();
	  ...
	  RichTextEditor.prototype.__toggleFormat__ = function (format) {
	  ...
	  RichTextEditor.prototype.__triggerEventListeners__ = function (event, eventData) {
	    ...
	    var NUM_OF_LISTENERS = LISTENERS.length;
	    for (var i = 0; i < NUM_OF_LISTENERS; i++) {
	      LISTENERS[i](eventData);
	    }
	  };
	  RichTextEditor.prototype.clearTextBoxLastSelectionData = function () {
	    this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection = null;
	    this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType = null;
	  };
	  RichTextEditor.prototype.applyColor = function (r, g, b) {
	    var _a;
	    // validate RGB values
	    var ARGS = [r, g, b];
	    if (ARGS.every(function (arg) {
	      return typeof arg === 'number';
	    }) === false || ARGS.every(function (arg) {
	    ...
	    COLOR_ELEMENT.css('color', "rgb(".concat(r, ",").concat(g, ",").concat(b, ")"));
	    // create event data
	    var COLOR_EVENT_DATA = {
	      textColor: {
	        r: r,
	        g: g,
	        b: b
	    ...
	    if (this.TEXT_BOX_LAST_SELECTION_DATA.lastSelection !== null && this.TEXT_BOX_LAST_SELECTION_DATA.lastSelectionType !== null) {
	      ...
	      if (SELECTION_TYPE === 'Caret') {
	        ...
	        if (isColorElement(SELECTED_ELEMENT)) {
	          ...
	          $(SELECTED_ELEMENT).css('color', "rgb(".concat(r, ",").concat(g, ",").concat(b, ")"));
	          this.__selectAndPlaceCaretInsideElement__(SELECTED_ELEMENT);
	        } else {
	        ...
	        this.__triggerEventListeners__('color', COLOR_EVENT_DATA);
	      } else if (SELECTION_TYPE === 'Range') {
	        if (isColorElement(SELECTED_ELEMENT)) {
	          ...
	          var PARENT_COLOR_ELEMENT = getClosestParentColorElement(COLOR_ELEMENT[0]);
	          if (PARENT_COLOR_ELEMENT !== undefined) {
	            separateColorElementFromParentColorElement(COLOR_ELEMENT[0], PARENT_COLOR_ELEMENT);
	          }
	          // check if the new color element has inner color elements and if so remove them leaving only their contents
	          var INNER_COLOR_ELEMENTS = getInnerColorElements$1(COLOR_ELEMENT[0]);
	          if (INNER_COLOR_ELEMENTS.length > 0) {
	            removeInnerColorElements(COLOR_ELEMENT[0]);
	            mergeSimilarAdjacentChildNodes(COLOR_ELEMENT[0]);
	          }
	          // check if the new color element is inside of <u> or <s> elements and if so take it out of the furthest ancestor
	          var FURTHEST_UNDERLINE_OR_STRIKETHROUGH_ELEMENT = getFurthestUnderlineOrStrikethroughAncestorElement(COLOR_ELEMENT[0]);
	          if (FURTHEST_UNDERLINE_OR_STRIKETHROUGH_ELEMENT !== undefined) {
	            separateColorElementFromUnderlineOrStrikethroughAncestorElement(COLOR_ELEMENT[0], FURTHEST_UNDERLINE_OR_STRIKETHROUGH_ELEMENT);
	          }
	          // get rid of any empty elements left
	          deleteAllEmptyDescendants(this.TEXT_BOX[0]);
	          // highlight selection again
	  ...
	  RichTextEditor.prototype.toggleBold = function () {
	    this.__toggleFormat__('b');
	  };
	  RichTextEditor.prototype.toggleItalic = function () {
	    this.__toggleFormat__('i');
	  };
	  RichTextEditor.prototype.toggleUnderline = function () {
	    this.__toggleFormat__('u');
	  };
	  RichTextEditor.prototype.toggleStrikethrough = function () {
	...
	window.addEventListener('load', function () {
	  var RTE = new RichTextEditor('text-box', false);
	  // FONT FORMATS
	  var FORMAT_BUTTONS = document.getElementById('font-formats');
	  if (FORMAT_BUTTONS instanceof HTMLElement) {
	    FORMAT_BUTTONS.addEventListener('click', function (event) {
	  ...
	  // COLOR PICKER
	  var color_picker_is_visible = false;
	  ...
	  if (COLOR_PICKER !== null && COLOR_PICKER instanceof HTMLInputElement && COLOR_PICKER.type === 'color') {
	    ...
	    COLOR_PICKER.addEventListener('input', function (event) {
	      if (event.target !== null && event.target instanceof HTMLInputElement) {
	        var HEX = event.target.value.substring(1); // ignore '#' at the start
	    ...
	    COLOR_PICKER.addEventListener('mousedown', function (event) {
	  ...
	  // HELPERS
	  function updateRTEToolWidgets(styles) {
	    var FORMATS_IN_SELECTION = styles.formats;
	    var NUM_OF_FORMATS = FORMATS_IN_SELECTION.length;
	    var BOLD_BUTTON = $('#bold');
	    var ITALIC_BUTTON = $('#italic');
	    var UNDERLINE_BUTTON = $('#underline');
	    var STRIKETHROUGH_BUTTON = $('#strikethrough');
	    ...
	    for (var i = 0; i < NUM_OF_FORMATS; i++) {
	    ...
	    if (COLOR_PICKER instanceof HTMLInputElement) {
	      ...
	      COLOR_PICKER.value = "#".concat(r).concat(g).concat(b);
	    }
	  }
	  // EVENT LISTENERS
	  RTE.addEventListener('mouseup', function (event) {
	    updateRTEToolWidgets(event.styles);
	  });
	  RTE.addEventListener('keyup', function (event) {
	    updateRTEToolWidgets(event.styles);
	  });
	  RTE.addEventListener('format', function (event) {
